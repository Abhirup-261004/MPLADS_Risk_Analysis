...
import { getAnalytics, getFilterOptions, getRiskCenter } from '../services/api.js';
...
export default function RiskCenter({ analyticsOnly = false }) {
  ...
  const [analyticsFilters, setAnalyticsFilters] = useState({ state: '', district: '', sector: '', agency: '' });
  const [filterOptions, setFilterOptions] = useState({ states: [], districts: [], sectors: [], agencies: [] });

  useEffect(() => {
    setError('');
    const request = analyticsOnly ? getAnalytics(analyticsFilters).then((analytics) => ({ analytics })) : getRiskCenter().then((center) => ({ center }));
    request.then(setData).catch(() => setError('Unable to load Risk Center intelligence. Please try again.'));
  }, [analyticsOnly, analyticsFilters]);
  useEffect(() => { if (analyticsOnly) getFilterOptions().then(setFilterOptions).catch(() => setFilterOptions({ states: [], districts: [], sectors: [], agencies: [] })); }, [analyticsOnly]);
  ...
  return <main className="risk-main"><header className="risk-top"><input placeholder="Search Work ID, Location or MP..." /><span>{analyticsFilters.state || 'India'}</span><b>PRAHARI Analyst</b></header><section className="risk-body">{analyticsOnly ? <Analytics data={data.analytics} filters={analyticsFilters} options={filterOptions} onChangeFilters={setAnalyticsFilters} /> : <PriorityQueue data={data.center} />}</section></main>;
}
...
function Analytics({ data, filters, options, onChangeFilters }) {
  ...
  const updateFilter = (key) => (event) => onChangeFilters({ ...filters, [key]: event.target.value, ...(key === 'state' ? { district: '' } : {}) });
  const filteredDistricts = filters.state ? options.districts : options.districts;
  return <section className="analytics-section"><header className="analytics-title"><div><h1>Analytics &amp; Insights</h1><p>Identify trends and patterns across MPLADS implementation.</p></div><div className="analytics-actions"><div className="analytics-filters"><select value={filters.state} onChange={updateFilter('state')}><option value="">India (All States)</option>{options.states.map((item) => <option key={item}>{item}</option>)}</select><select value={filters.district} onChange={updateFilter('district')}><option value="">All Districts</option>{filteredDistricts.map((item) => <option key={item}>{item}</option>)}</select><select value={filters.sector} onChange={updateFilter('sector')}><option value="">All Sectors</option>{options.sectors.map((item) => <option key={item}>{item}</option>)}</select><select value={filters.agency} onChange={updateFilter('agency')}><option value="">All Agencies</option>{options.agencies.map((item) => <option key={item}>{item}</option>)}</select></div><div className="analytics-downloads"><button onClick={() => downloadSvg('expenditure-trend.svg', 'MPLADS Expenditure Trend', series.map((point) => ({ label: point.month, value: point.value })))}>Download Trend SVG</button><button onClick={() => downloadSvg('risk-volatility.svg', 'MPLADS Risk Volatility', series.map((point) => ({ label: point.month, value: point.volatility })))}>Download Volatility SVG</button></div></div></header><div className="analytics-grid">
